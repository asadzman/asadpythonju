...
class StringInsight:
    ...
    def multi_length_count(self):
        return (
            f"Method 1: Total Characters: {len(self.text)}\n"
            f"Method 2: Non-Space Characters: {len(self.text.replace(' ', ''))}\n"
            f"Method 3: Total Word Count: {len(self.words)}\n"
            f"Method 4: Unique Word Count: {len(set(self.words))}"
        )


# Example usage:
example_text = input("Please enter a string: ")
